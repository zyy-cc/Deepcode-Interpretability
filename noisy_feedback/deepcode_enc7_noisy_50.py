# Deepcode: encoder with 7 hidden states and decoder with 7 hidden states
# noisy feedback 
...
def get_args(jupyter_notebook):
    ...
    parser.add_argument('-enc_num_unit', type=int, default=7)
    parser.add_argument('-dec_num_unit', type=int, default=7)

    parser.add_argument('-feedback_SNR', type=int, default=10, help='100 means noiseless feedback')
...
class AE(torch.nn.Module): 
    def __init__(self, args):
        ...
        # Encoder
        self.enc_rnn = torch.nn.RNN(input_size=4, hidden_size=args.enc_num_unit, num_layers=1, nonlinearity='tanh', dropout=0, bias=True, batch_first=True, bidirectional = False)
    
        self.enc_linear    = torch.nn.Linear(in_features=args.enc_num_unit, out_features=2, bias=True) # Generate two codewords per cell.
    ...
    def forward(self, input, forward_noise, feedback_noise):

        num_samples_input = input.shape[0]

        # encoder part: Phase 1

        codewords_phase_1 = 2.0 * input-1.0

        # encoder part: Phase 2

        all_hidden_states = []
        for idx_bit in range(input.shape[1]):
            
            if idx_bit == 0:
                noise_tmp = forward_noise[:,idx_bit,:] + feedback_noise[:,idx_bit,:]
                noise_tmp[:,1:] = torch.zeros(noise_tmp[:,1:].shape)

                input_tmp           =   torch.cat([input[:,idx_bit,:].view(num_samples_input, 1, 1),
                                                   noise_tmp.view(num_samples_input, 1, 3)], dim=2)

                rnn_output, h_tmp  = self.enc_rnn(input_tmp) # initial with zero h_tmp
                all_hidden_states.append(h_tmp)
            else:
                noise_tmp = forward_noise[:,idx_bit,:] + feedback_noise[:,idx_bit,:]
                ...
                rnn_output, h_tmp  = self.enc_rnn(input_tmp, h_tmp)
                all_hidden_states.append(h_tmp)

            # linear combination
            dense_output = torch.sigmoid(self.enc_linear(rnn_output))

            if idx_bit == 0:
                codewords_phase_2= dense_output.view(num_samples_input, 1, 2)
            else:
                codewords_phase_2 = torch.cat([codewords_phase_2,dense_output ], dim = 1)
...
args.initial_weights = 'weight/model_enc7_snr0_fbsnr10_50.pt'
