# Interpretable model with varying knee points
# noisy feedback 
...
class AE(torch.nn.Module): 
    def __init__(self, args):
        ...
        # Encoder
        self.e = torch.nn.Parameter(torch.rand(4), requires_grad = True) # e[1]: \lambda_1, e[2]: \lambda 2
    ...
    def forward(self, information_bits, forward_noise, feedback_noise):
        ...
        def parity1(b, n1):
            ...
            n_i
            """
            offset = torch.where(b == 0, self.e[1], - self.e[2])
        ...
        for idx_bit in range(information_bits.shape[1]):
            ...
            # parity bits
            p1 = p1_linear - self.e[3] * vh4 - self.e[3] * vh5
            
            p2 = p2_linear - self.e[3] * vh4 - self.e[3] * vh5
