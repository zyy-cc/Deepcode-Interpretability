...
def train(args, model, device, optimizer, scheduler):  
    ...
    for __ in range(num_batch):
        ...
        # torch.zeros((args.batch_size, args.block_len+1, args.code_rate))   # perfect feedback
    ...
    def forward(self, information_bits, forward_noise, feedback_noise):
        ...
        for idx_bit in range(information_bits.shape[1]):
            ...
            if idx_bit == 0:
                ...
                past_n1 = forward_noise[:,idx_bit-1,0].view(num_samples_input, 1, 1) + feedback_noise[:,idx_bit-1,0].view(num_samples_input, 1, 1)
...
args.initial_weights = 'weight/model_sym2_snr0_noiseless.pt'
...
#         file_name = './weight/model_'+date.today().strftime("%Y%m%d")+'_'+identity+'.pt'
#         torch.save(model.state_dict(), file_name)
#         print('saved model as file: ', file_name)

#         file_name = './weight/model_'+date.today().strftime("%Y%m%d")+'_'+identity+'_args.json'
