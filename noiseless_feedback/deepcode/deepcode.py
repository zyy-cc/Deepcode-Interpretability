...
def get_args(jupyter_notebook):
    ...
    # change the number of hidden states here
    parser.add_argument('-enc_num_unit', type=int, default=5)
...
class AE(torch.nn.Module): 
    ...
    def forward(self, input, forward_noise, feedback_noise):
        ...
        for idx_bit in range(input.shape[1]):
            ...
            # norm_output  = self.normalize(dense_output)
...
# enc(x): x hidden states in encoder and decoder
# snr: forward SNR
args.initial_weights = 'weight/5hidden/model_enc5_snr0.pt'
if args.initial_weights == 'default':
    ...
    model.load_state_dict(torch.load(args.initial_weights, map_location='cpu'))
    model.args = args
